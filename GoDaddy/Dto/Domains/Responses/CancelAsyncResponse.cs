...
public sealed class CancelAsyncResponse
{
    ...
    public CancelAsyncResponse(JsonNode? raw, JsonObject? @object, JsonArray? items, string? code, string? message)
    {
        Raw = raw;
        Object = @object;
        Items = items;
    ...
    public static CancelAsyncResponse FromJson(JsonNode? raw)
    {
        ...
        return new CancelAsyncResponse(raw, @object, items, code, message);
    }
}
